...
// ✅ Updated schema with matching field names and confirm password logic
const formSchema = z
  .object({
    newPassword: z
      .string()
...
function ResetPasswordPage() {
  ...
  const onSubmit = (data: FormValues) => {
    ...
    // Add reset logic here (API call, toast, etc.)
    setIsLoading(true);
    setTimeout(() => {
          ...
          >
            ...
            <Button
              ...
              className={`w-full h-16 md:h-14 rounded-full text-lg md:text-base lg:text-lg font-bold text-white
               ...
               shadow-[0_6px_0_0_#c2334a] hover:brightness-110 transition-all duration-300 ${isLoading ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
              disabled={isLoading}
            >
              {isLoading ? (
                <>
